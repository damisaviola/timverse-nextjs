    ...
    <div className="w-full max-w-[440px] bg-card/60 backdrop-blur-2xl shadow-2xl border border-border/40 rounded-2xl sm:rounded-[2rem] p-5 sm:p-8 md:p-10 relative z-10">
      ...
      <div className="mb-6 sm:mb-8">
         ...
         <p className="text-sm text-secondary font-medium leading-relaxed">
           Selamat datang kembali di TIMVERSE.
         </p>
      </div>

      {/* Success Message */}
      {isSuccess && (
        <div className="mb-6 p-4 bg-emerald-500/10 border border-emerald-500/20 rounded-xl flex items-center gap-3 text-emerald-500 text-xs font-bold animate-in fade-in slide-in-from-top-2">
      ...
      <form action={formAction} className="space-y-5">
        <div className="space-y-2">
          ...
          <div className="relative group">
             <Mail size={16} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-muted group-focus-within:text-accent transition-colors" />
             <input 
               name="email"
               type="email"
        ...
        <div className="space-y-2">
          ...
          <div className="relative group">
             <Lock size={16} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-muted group-focus-within:text-accent transition-colors" />
             <input 
               name="password"
               type="password"
          ...
          <div className="flex justify-end">
            <Link href="#" className="text-[10px] font-black text-accent uppercase tracking-widest hover:underline px-1">Lupa Sandi?</Link>
          </div>
        </div>
        ...
        <button 
          type="submit"
          disabled={isPending}
          className="w-full flex items-center justify-between bg-foreground text-card px-5 py-3.5 sm:py-4 rounded-xl sm:rounded-2xl text-[11px] font-black uppercase tracking-widest hover:bg-accent hover:text-white transition-all group disabled:opacity-50"
        >
          {isPending ? (
            <>
              Sedang Masuk...
  ...
  return (
    <div className="min-h-dvh bg-background flex flex-col items-center justify-center px-4 py-8 sm:py-12 relative overflow-hidden">
      ...
      <div className="fixed inset-0 pointer-events-none overflow-hidden -z-10">
        <div className="absolute top-[-5%] left-[-5%] w-[400px] sm:w-[600px] h-[400px] sm:h-[600px] bg-accent/15 rounded-full blur-[140px]" />
        <div className="absolute bottom-[-5%] right-[-5%] w-[400px] sm:w-[600px] h-[400px] sm:h-[600px] bg-secondary/15 rounded-full blur-[140px]" />
      </div>

      <Suspense fallback={
        <div className="w-full max-w-[440px] bg-card/60 backdrop-blur-2xl shadow-2xl border border-border/40 rounded-2xl p-10 flex items-center justify-center">
